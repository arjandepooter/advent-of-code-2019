...
setInput Runtime {..} ips = Runtime program pointer (inputs ++ ips) outputs
...
runAmplifierLoop prg codes =
  ...
  [0 ..]
  where
    runtimes = fmap (\c -> Runtime prg 0 [c] []) codes
    f :: (Int, Bool, [Runtime]) -> a -> (Int, Bool, [Runtime])
    f (input, _, rt:rts) _ =
      let (finished, Runtime {..}) =
            runState runUntilOutput (setInput rt [input])
       in (head outputs, finished, rts ++ [Runtime program pointer [] []])
